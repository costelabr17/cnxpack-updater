    ...
    if (this->draw_page) {
        if (!this->workStarted) {
            ...
            if (ProgressEvent::instance().getTotal()) {
                this->label->setText(fmt::format("{0} ({1:.1f} MB de {2:.1f} MB - {3:.1f} MB/s)", text, ProgressEvent::instance().getNow() / 0x100000, ProgressEvent::instance().getTotal() / 0x100000, ProgressEvent::instance().getSpeed() / 0x100000));
            }
